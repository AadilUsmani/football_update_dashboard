...
export async function intentClassifierNode(state: AgentGraphStateType): Promise<Partial<AgentGraphStateType>> {
  ...
  const cricketKeywords = ['cricket', 'psl', 'ipl', 'bbl', 'test', 't20', 'odi', 'pakistan', 'india', 'babar', 'shaheen', 'kohli', 'csk', 'lahore', 'karachi kings', 'islamabad united', 'multan'];
  ...
  else if (q.includes('india') && !q.includes('vs india')) detectedCountry = 'India';
  else if (q.includes('uae') || q.includes('dubai')) detectedCountry = 'UAE / Middle East';
  else if (q.includes('pakistan') && !q.includes('vs pakistan')) detectedCountry = 'Pakistan';
...
export async function fixtureRetrieverNode(state: AgentGraphStateType): Promise<Partial<AgentGraphStateType>> {
  ...
  if (matched.length === 0) {
    // fallback to top upcoming matches
    matched = allMatches.slice(0, 3);
  }

  const traceStep: TraceStep = {
    node: 'FixtureRetriever',
    description: `Retrieved ${matched.length} matching upcoming fixtures across selected sport database`,
...
export async function broadcastResolverNode(state: AgentGraphStateType): Promise<Partial<AgentGraphStateType>> {
  ...
  const traceStep: TraceStep = {
    ...
    description: `Resolved official broadcasting rights for ${country} across ${matches.length} fixtures`,
    timestamp: new Date().toISOString(),
...
export async function timezoneNormalizerNode(state: AgentGraphStateType): Promise<Partial<AgentGraphStateType>> {
  ...
  const traceStep: TraceStep = {
    ...
    description: `Converted all match kickoffs to target timezone: [${tz}] (${tz === 'Asia/Karachi' ? 'Pakistan Standard Time (PKT UTC+5)' : tz})`,
    timestamp: new Date().toISOString(),
...
export async function insightSynthesizerNode(state: AgentGraphStateType): Promise<Partial<AgentGraphStateType>> {
  const { matchedMatches, entities, query, intent } = state;
  const country = entities?.country || 'Pakistan';
  const tz = entities?.timezone || 'Asia/Karachi';

  if (!matchedMatches || matchedMatches.length === 0) {
    const defaultMsg = `I couldn't find a direct fixture match for "${query}". You can browse all upcoming Premier League, MLS, La Liga, Champions League, PSL, and International Cricket matches right on the dashboard!`;
    return {
  ...
  let answer = `### ⚽/🏏 Next Match Intelligence\n\n`;
  answer += `**${primaryMatch.homeTeam.name} vs ${primaryMatch.awayTeam.name}**\n`;
  answer += `🏆 **Tournament:** ${primaryMatch.competitionName} (${primaryMatch.formatOrStage})\n`;
  ...
  answer += `#### 📺 Where to Watch in ${country}:\n`;
  channels.forEach(ch => {
    answer += `- 📡 **${ch}**\n`;
  });

  if (primaryMatch.watchUrlsByCountry?.[country]) {
    answer += `\n🔗 **Direct Stream:** [Watch on Official Provider](${primaryMatch.watchUrlsByCountry[country]})\n`;
  }
  ...
  if (primaryMatch.aiMatchInsight) {
    answer += `\n**💡 Tactical Insight:** ${primaryMatch.aiMatchInsight}\n`;
  }

  if (matchedMatches.length > 1) {
    answer += `\n---\n#### 📅 Other Upcoming Related Matches:\n`;
    matchedMatches.slice(1, 4).forEach(m => {
      ...
      answer += `- **${m.homeTeam.name} vs ${m.awayTeam.name}** (${m.competitionName}) • *${time}* • 📺 ${chs[0] || 'Check Guide'}\n`;
    });
  }

  const traceStep: TraceStep = {
    node: 'InsightSynthesizer',
    description: `Synthesized comprehensive match breakdown, PKT timings, broadcast routing, and AI tactical insights`,
    timestamp: new Date().toISOString(),
    outputSummary: `Generated final structured intelligence card for ${primaryMatch.homeTeam.name} vs ${primaryMatch.awayTeam.name}`
  };
